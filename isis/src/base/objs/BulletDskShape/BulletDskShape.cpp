 ...
 */
  void BulletDskShape::loadFromDsk(const QString &dskfile) {
    ...
    // so ensure each segment shape can fit in a single Bullet part. 
    //
    ...
    while(found) {
      ...
      if ( s_plates > max_bt_triangles ) {
        QString mess = "Segment " + toString( segments.size()-1) + " number triangles ("
      ...
      Spiceint s_parts = (s_plates + max_bt_triangles - 1) / max_bt_triangles;
      nparts_required += nparts; 
